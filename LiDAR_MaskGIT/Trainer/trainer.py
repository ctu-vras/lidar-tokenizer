...
from Dataset.dataset import FrontCamTokens
...
class Trainer(object):
    ...
    def get_data(self):
        ...
        if self.args.data == "frontcam":
            # load both img token and lidar token 
            dataroot = self.args.data_folder
            info_file_path = self.args.data_info
            data_train = FrontCamTokens(dataroot, version='v1.0-trainval', split='train', info_file_path=info_file_path, lidar_tokens_offset=self.args.codebook_size_img, device=self.args.device)
            data_test = FrontCamTokens(dataroot, version='v1.0-trainval', split='test', info_file_path=info_file_path, lidar_tokens_offset=self.args.codebook_size_img, device=self.args.device)
    ...
    @staticmethod
    def get_optim(net, lr, mode="AdamW", num_train_samples=60960, onecycle=False, epochs=21, bs=8, **kwargs):
        ...
        if mode == "AdamW":
            if onecycle:
                opt =  optim.AdamW(params, lr, weight_decay=0.03, **kwargs)
            
                scheduler = torch.optim.lr_scheduler.OneCycleLR(
                            opt,
                            max_lr = lr,
                            epochs = int(epochs),
                            steps_per_epoch = int(num_train_samples/bs),
                            pct_start=0.3,
                            anneal_strategy="cos",
                            div_factor=10.0,
                ...
                return optim.AdamW(params, lr, weight_decay=0.03, **kwargs)
    ...
    def save_network(self, model, path, iter=None, optimizer=None, lr_scheduler=None, global_epoch=None):
